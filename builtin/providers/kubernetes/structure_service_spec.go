...
func flattenServiceSpec(in v1.ServiceSpec) []interface{} {
	...
	if len(in.Ports) > 0 {
		att["ports"] = flattenServicePort(in.Ports)
	}
...
func expandServicePort(l []interface{}) []v1.ServicePort {
	...
	for i, n := range l {
		...
		if v, ok := cfg["target_port"].(int); ok {
			obj[i].TargetPort = expandIntOrString(v)
		}
...
func expandServiceSpec(l []interface{}) v1.ServiceSpec {
	...
	obj := v1.ServiceSpec{}
	if v, ok := in["ports"].(*schema.Set); ok && v.Len() > 0 {
		obj.Ports = expandServicePort(v.List())
	}
